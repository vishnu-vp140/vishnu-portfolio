...
const TIMELINE = [
  ...
  },
  {
    ...
    desc: "Completed the Dante Pro Lab, an advanced network penetration testing environment simulating a real enterprise. Achieved global rank #938.",
    ...
    <section id="timeline" className="py-24 px-6">
      <div className="max-w-3xl mx-auto" ref={ref}>
        <div className="reveal">
          <SectionHeader label="journey" title="Timeline" />
          <p className="text-white/30 text-sm mb-14 -mt-8 font-medium">
            Certifications, education, competitions and events.
          </p>
        </div>
          ...
          <div className="flex flex-col">
            {TIMELINE.map((item, i) => (
              <div key={i} className="reveal relative flex gap-6 pb-9 group">
                ...
                <div className="flex-1 bg-white/[0.03] border border-white/[0.07] rounded-2xl p-5 hover:bg-white/[0.055] transition-all duration-300 group-hover:-translate-y-0.5">
                  <div className="flex flex-wrap items-center gap-2 mb-3">
                    <span className="font-mono text-xs text-white/25 tracking-wider">{item.date}</span>
                    <span className={"text-[0.65rem] font-semibold px-2.5 py-0.5 rounded-full border " + TYPE_BADGE[item.type]}>
                      {TYPE_LABEL[item.type]}
                    </span>
                    {item.highlight && (
                      <span className="text-[0.65rem] font-bold px-2.5 py-0.5 rounded-full bg-yellow-500/15 text-yellow-300 border border-yellow-500/20">
                        Key Achievement
                      </span>
                    )}
                  </div>
                  <h3 className="font-bold text-white text-base leading-snug">{item.title}</h3>
                  <p className="text-white/30 text-xs font-medium mt-0.5 mb-3">{item.org}</p>
                  <p className="text-white/45 text-sm leading-relaxed mb-4">{item.desc}</p>
                  <div className="flex flex-wrap gap-2">
                    {item.tags.map((tag) => (
                      <span key={tag} className="text-[0.65rem] font-medium px-2.5 py-1 rounded-lg bg-white/[0.04] text-white/35 border border-white/[0.07]">
                        {tag}
                      </span>
                    ))}
                  </div>
